...
int64 nChainStartTime = 1431604800;
...
const string strMessageMagic = "SilverBullet Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(nHeight <= 5500)
	nSubsidy = 50 * COIN; 

    else if(nHeight <= 5501)
	nSubsidy = 550 * COIN;
    ...
    else if(nHeight <= 6500)
	nSubsidy = 50 * COIN; 

    else if(nHeight <= 6501)
	nSubsidy = 650 * COIN; 

    else if(nHeight <= 7000)
	nSubsidy = 50 * COIN;

    else if(nHeight <= 7001)
	nSubsidy = 3000 * COIN; 

    else if(nHeight <= 7500)
	nSubsidy = 50 * COIN; 

    else if(nHeight <= 7501)
	nSubsidy = 750 * COIN; 
    ...
    else if(nHeight <= 9500)
	nSubsidy = 50 * COIN;

    else if(nHeight <= 9501)
	nSubsidy = 950 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_SVB_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SilverBullet
    bool fStrictPayToScriptHash = true; // Always active in SilverBullet
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SilverBullet", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1431604800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1021379;
        ...
        assert(block.hashMerkleRoot == uint256("0x21e4793723b88e5f30c638db5f1b2750c6cb942ba07782ffe0371fc501634cea"));
